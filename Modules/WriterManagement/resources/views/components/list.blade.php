            ...
            <x-adminlte-datatable id="table8" :heads="$heads" head-theme="dark" striped :config="$config"
                ...
                @foreach ($writers as $index => $row)
                    <tr>
                        ...
                        <td>
                            <a @if ($row->status == 1)href="{{ route('writermanagement.edit', $row->id) }}" @else href="javascript:function() { return false; }" @endif><button
                            ...
                            @else
                                <a href="{{ route('writermanagement.disableEnableWriter', $row->id) }}"><button
                                        class="btn btn-xs btn-success  mx-1 shadow" title="Enable">Enable</button>
                            @endif
