...
<div class="{{ $layoutHelper->makeContentWrapperClasses() }}">
    ...
    <div class="content" style="padding-top: 20px;margin-left: 10px">
        <x-adminlte-card title="Show Job Register" theme="success" icon="fas fa-lg fa-person">
            <form>
                <div class="row pt-2">
                    <x-adminlte-select name="client_id" id="client_id" fgroup-class="col-md-6" disabled>
                        <option value="">Select Client</option>
                        @foreach ($clients as $client)
                            <option value="{{ $client->id }}" {{ $jobRegister->client_id == $client->id ? 'selected' : '' }}>{{ $client->name }}</option>
                        @endforeach
                    </x-adminlte-select>
                    <x-adminlte-input name="client_contact_person_id" placeholder="Client Contact Person" fgroup-class="col-md-6" value={{$contact_person}} disabled/>
                    
                    
                    <x-adminlte-select2 name="estimate_id" fgroup-class="col-md-6" disabled :config="$config">
                    ...
                    <x-adminlte-select name="handled_by_id" fgroup-class="col-md-6" disabled>
                    ...
                    <x-adminlte-select name="client_accountant_person_id" fgroup-class="col-md-6" disabled>
                    ...
                    <x-adminlte-textarea name="other_details" placeholder="Other Details" fgroup-class="col-md-6" disabled>{{ $jobRegister->other_details }}</x-adminlte-textarea>

                    <x-adminlte-select name="type" fgroup-class="col-md-6" id="type" disabled>
                    ...
                    <x-adminlte-select name="language" fgroup-class="col-md-6" disabled>
                    ...
                    <x-adminlte-input name="protocol_no" placeholder="Protocol Number" fgroup-class="col-md-6" value="{{ $jobRegister->protocol_no }}" disabled/>

                    <x-adminlte-input name="date" placeholder="Date" fgroup-class="col-md-6" type="date" value="{{ $jobRegister->date }}" disabled/>

                    <x-adminlte-textarea name="description" placeholder="HEADING / DESCRIPTION" fgroup-class="col-md-6" disabled>{{ $jobRegister->description }}</x-adminlte-textarea>
